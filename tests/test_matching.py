...
def test_exact_utr_match_found():
    ...
    candidates, match_type, evidence = find_bank_candidates(
        pg,
        index,
    )
...
def test_fuzzy_fallback_recovers_corrupted_utr():
    """
    Fuzzy tier specifically: bank has NO resolved txn_id and a
    corrupted UTR. This forces the search past tiers 1 and 2,
    into the guarded fuzzy fallback.
    """
...
def test_fuzzy_never_fires_without_amount_date_agreement():
    """
    Even with identical narration text, fuzzy must NOT fire if
    amount disagrees.

    Bank has no resolved txn_id, so this genuinely forces the search
    into the fuzzy tier rather than short-circuiting through exact_txn.
    """
    ...
    candidates, match_type, _ = find_bank_candidates(
        pg,
        index,
    )
...
def test_fuzzy_guard_compares_expected_net_not_raw_gross():
    """
    Regression test for the gross-vs-net comparison bug.

    The fuzzy tier's amount guard must compare the PG record's
    EXPECTED NET:

        gross - fee - gst - tds

    against the bank's net amount.
    """
    ...
    assert len(candidates) == 1, (
        "Expected the genuine candidate to pass the amount "
        "guard using expected net, not be rejected by "
        "comparing raw gross to net"
    )
...
def test_invoice_candidate_exact_txn_only():
    ...
    candidates, match_type, evidence = find_invoice_candidates(
        pg,
        index,
    )
...
def test_missing_source_returns_empty_not_error():
    """
    missing_in_source category: no candidate should raise.
    It should cleanly return an empty result for the decision engine
    to classify.
    """
    ...
    bank_candidates, bank_type, _ = find_bank_candidates(
        pg,
        index,
    )

    invoice_candidates, invoice_type, _ = find_invoice_candidates(
        pg,
        index,
    )
...
def test_missing_bank_still_scores_meaningfully():
    """
    PARTIAL_MATCH precursor: invoice present, bank absent.
    """
...
def test_stale_tax_rate_breaks_fee_consistency_not_amount_match():
    """
    PG and bank still agree on settlement amounts, but invoice fee
    diverges from PG fee.
    """
    ...
    assert score.amount_matched_bank is True
...
def test_ambiguous_candidates_resolved_deterministically_and_repeatably():
    """
    Two equally plausible bank candidates.

    Selection must be deterministic:
        1. same winner every run
        2. independent of arbitrary list order
        3. rejected candidate preserved

    The candidates deliberately have different UTR values so they remain
    distinct records in the candidate-generation/indexing layer.
    """
    ...
    results_run_1 = run_matching(
        normalized_pool
    )

    results_run_2 = run_matching(
        normalized_pool
    )
...
def test_ambiguous_result_never_auto_matchable():
    """
    Even if the chosen candidate's raw score would qualify as HIGH,
    genuine ambiguity must demote confidence to LOW.

    IMPORTANT:
    The two bank candidates must have distinct UTRs. If both candidates
    share the same UTR, candidate indexing may treat them as the same
    exact reference and the test would no longer represent genuine
    candidate ambiguity.
    """
    ...
    assert r.bank_candidate_count == 2
    assert len(r.rejected_bank_candidates) == 1
...
def test_match_result_invariants_hold():
    ...
    assert results[0].sources_present == [
        "pg"
    ]
...
def test_missing_in_source_category_produces_no_candidate_not_crash():
    ...
    missing_bank_or_invoice = [
        ...
        for r in results
        if (
            r.bank_record is None
            or r.invoice_record is None
        )
    ]
    ...
    assert len(
        ...
        "Expected at least one transaction with a genuinely "
        "missing bank or invoice record -- missing_in_source category."
    )
